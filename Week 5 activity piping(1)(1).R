...
#solution
mysample2<-mysample%>%arrange(mysample2,airline)
  rename(mysample2, seats=avail_seat_km_per_week)%>%print()

mysample3<-mysample%>%arrange(airline)%>%
  select(avail_seat_km_per_week,incidents_00_14,incidents_85_99)%>%print()

mysample4<-mysample%>%subset(incidents_85_99<25)%>%
  select(incidents_00_14,incidents_85_99)%>%summary()%>%print()
